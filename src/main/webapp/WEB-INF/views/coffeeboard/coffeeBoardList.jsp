	...
	<body class="is-preload">
		<main style="width: 80%; margin: 0 auto;">
			...
			<div style="text-align: center; margin: 0 auto;">
				<h1 style="padding-top:1rem;">커피 게시판</h1>
				<hr>
			</div>
			<div style="width: 100%; height: 3rem;">
	    		<form action="/coffee/coffeeboard/coffeeBoardList" method="post">
			...
			<table class="table table-striped" style="margin: 0 auto;">
				...
				<c:set var="no" value="${list.no}" />
					...
					<c:set var="index" value="${loop.index + 1}" />
						<tr class="boardArticles" onclick="location.href='${path}/coffee/coffeeboard/coffeeBoardDetail/${vo.no}'">
							<td class="boardNo">${index + ((list.currentPage-1) * 10)}</td>
							<td class="boardImg">
								<img src="${path}/coffee/coffeeboard/thumbnail?no=${vo.no}">
							</td>
			...
			<div style="display: inline-block; width: 100%; margin: 1em 0 -1em;">
				<c:if test="${not empty member}">
					<button class="cssbuttons-io-button writeBtn" onclick="location.href='/coffee/coffeeboard/coffeeBoardForm'">
						글작성
						<div class="icon">
						   <svg
						     height="24"
				...
				<c:if test="${not empty list.vo}">
					...
					<ul class="pagination" style="float: right; margin-right: 20px">
						...
						<c:if test="${startPage > pageBlock}">
							<li class="page-item">
								<a class="page-link" href="${path}/coffee/coffeeboard/coffeeBoardList?pageNum=${startPage - pageBlock}
						...
						<c:forEach var="n" begin="${startPage}" end="${endPage}">
							<c:choose>
								<c:when test="${n == list.currentPage}">
									<li class="page-item active"><a class="page-link" href="${path}/coffee/coffeeboard/coffeeBoardList?pageNum=${currentPage}
									<c:if test="${searching.keyField != ''}">&keyField=${searching.keyField}</c:if>
									<c:if test="${searching.keyword != ''}">&keyword=${searching.keyword}</c:if>
									">${list.currentPage}</a></li>
								</c:when>
								<c:otherwise>
									<li class="page-item"><a class="page-link" href="${path}/coffee/coffeeboard/coffeeBoardList?pageNum=${n}
									<c:if test="${searching.keyField != ''}">&keyField=${searching.keyField}</c:if>
									<c:if test="${searching.keyword != ''}">&keyword=${searching.keyword}</c:if>
									">${n}</a></li>
								</c:otherwise>
