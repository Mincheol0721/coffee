...
/* 				border-bottom: 1px solid lightgray; */
	...
	<body>
		<div id="whole">
			...
			<hr>
			<div id="btns">
				<button onclick="location.href='/coffee/coffeeboard/coffeeBoardList'" class="bn59">글 목록</button>
				<c:if test="${member.id eq owner.id}">
					<button onclick="location.href='/coffee/coffeeboard/modCoffeeBoardForm/${vo.no}'" class="bn59">글 수정</button>
					<button onclick="location.href='/coffee/coffeeboard/delCoffeeBoard/${vo.no}'" class="bn59">글 삭제</button>
				</c:if>
			</div>
			<hr>
			<div id="commentBox">
								...
								<div style="width: 100%;">
									<div class="commentListProfile">
										...
										<c:if test="${list.nickname eq member.nickname}">
											...
											<small><a href="/coffee/coffeeboardComment/delComment?no=${list.no}&boardNo=${vo.no}"><i class="fa-regular fa-trash-can"></i></a></small>
										</c:if>
									</div>
									...
									<div class="recFrm hide" style="display: none;">
										<form action="/coffee/coffeeboardComment/regComment" method="post">
				...
				<div id="input-wrapper" style="height: 125px; border: 1px solid lightsteelblue;">
					...
					<form action="/coffee/coffeeboardComment/regComment" method="post" style="display: flex;justify-content: space-between;flex-wrap: nowrap;">
						<input type="hidden" name="boardNo" value="${vo.no}">
						<input type="hidden" name="nickname" value="${member.nickname}">
...
// 				console.log(content.nextElementSibling);
			...
			function modComment(no, boardNo, idx) {
				...
				$.ajax({
					url: '/coffee/coffeeboardComment/modComment',
					type: 'POST',
					dataType: 'text',
