	...
	<body class="is-preload">
		<main style="width: 80%; margin: 0 auto;">
			...
			<div style="width: 100%; height: 3rem;">
	    		<form action="/qnaboard/qnaBoardList" method="post">
	    		...
	    		<button class="viewAll" onclick="location.href='/coffeeboard/coffeeBoardList'">전체보기</button>
			</div>
			<br>
			<table class="table table-striped" style="margin: 0 auto;">
				...
				<c:set var="no" value="${list.no}" />
					...
					<c:set var="index" value="${loop.index + 1}" />
						<tr class="boardArticles" onclick="location.href='${path}/qnaboard/qnaBoardDetail?no=${vo.no}'">
							<td class="boardNo">${index + ((list.currentPage-1) * 10)}</td>
							<td class="boardImg">
								<img src="${path}/qnaboard/thumbnail?no=${vo.no}">
							</td>
			...
			<div style="display: inline-block; width: 100%; margin: 1em 0 -1em;">
				<c:if test="${not empty member}">
					<button class="cssbuttons-io-button writeBtn" onclick="location.href='/qna/qnaForm'">
						글작성
						<div class="icon">
						   <svg
						     height="24"
				...
				<c:if test="${not empty list.vo}">
					...
					<ul class="pagination" style="float: right; margin-right: 20px">
						...
						<c:if test="${startPage > pageBlock}">
							<li class="page-item">
								<a class="page-link" href="${path}/qnaboard/qnaBoardList?pageNum=${startPage - pageBlock}
						...
						<c:forEach var="n" begin="${startPage}" end="${endPage}">
							<c:choose>
								<c:when test="${n == list.currentPage}">
									<li class="page-item active"><a class="page-link" href="${path}/qnaboard//qnaBoardList?pageNum=${currentPage}
									<c:if test="${not empty category}">&category=${category}</c:if>
									<c:if test="${searching.keyField != ''}">&keyField=${searching.keyField}</c:if>
									<c:if test="${searching.keyword != ''}">&keyword=${searching.keyword}</c:if>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link" href="${path}/qnaboard/qnaBoardList?pageNum=${n}
									<c:if test="${not empty category}">&category=${category}</c:if>
									<c:if test="${searching.keyField != ''}">&keyField=${searching.keyField}</c:if>
									<c:if test="${searching.keyword != ''}">&keyword=${searching.keyword}</c:if>
									">${n}</a></li>
...
<%-- 						<c:out value="pageBlock: ${pageBlock} / pageSize: ${pageSize} / count: ${count} / pageCount: ${pageCount} / pageGroup: ${pageGroup} / startPage: ${startPage} / endPage: ${endPage}" />  --%>
					...
					<%--
						...
						<c:if test="${startPage > pageBlock}">
							<li class="page-item">
								<a class="page-link" href="${path}/qnaboard/qnaBoardList?
									<c:if test="${not empty category}">category=${category}&</c:if>
						...
						<c:forEach var="n" begin="${startPage}" end="${endPage}">
							<c:choose>
								<c:when test="${n == list.currentPage}">
									<li class="page-item active"><a class="page-link" href="${path}/qnaboard/qnaBoardList?
									<c:if test="${not empty category}">category=${category}&</c:if>
									<c:if test="${searching.keyField != null || searching.keyField != ''}">keyField=${searching.keyField}&</c:if>
									<c:if test="${searching.keyword != null || searching.keyword != ''}">keyword=${searching.keyword}&</c:if>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link" href="${path}/qnaboard/qnaBoardList?
									<c:if test="${not empty category}">category=${category}&</c:if>
									<c:if test="${searching.keyField != null || searching.keyField != ''}">keyField=${searching.keyField}&</c:if>
									<c:if test="${searching.keyword != null || searching.keyword != ''}">keyword=${searching.keyword}&</c:if>
									pageNum=${n}">${n}</a></li>
