...
<%-- 				    <c:out value="owner ID : ${owner.id}" /> --%>
			...
			<hr>
			<div id="btns">
				<button onclick="location.href='/dailyBoard/dailyBoardList'" class="bn59">글 목록</button>
				<c:if test="${member.id eq owner.id}">
					<button onclick="location.href='/dailyBoard/modDailyBoardForm?no=${vo.no}'" class="bn59">글 수정</button>
					<button onclick="location.href='/dailyBoard/delDailyBoard?no=${vo.no}'" class="bn59">글 삭제</button>
				</c:if>
			</div>
			<hr>
			<div id="commentBox">
						...
						<c:forEach var="list" items="${commentList}" varStatus="loop">
								...
								<div style="width: 100%;">
									<div class="commentListProfile">
										<img alt="profile" src="/member/download?nickname=${list.nickname}" style="width: 50px; height: 50px; border-radius: 60%; float: left; object-fit: cover; border: 1px solid lightgray;">
										...
										<c:if test="${list.nickname eq member.nickname}">
											...
											<small><a href="/dailyBoardComment/delComment?no=${list.no}&boardNo=${vo.no}"><i class="fa-regular fa-trash-can"></i></a></small>
										</c:if>
									</div>
									...
									<div class="recFrm hide" style="display: none;">
										<form action="/dailyBoardComment/regComment" method="post">
				...
				<div id="input-wrapper" style="height: 125px; border: 1px solid lightsteelblue;">
					<c:if test="${member ne null}">
						<img alt="profile" src="/member/download?nickname=${member.nickname}" class="profile">
					...
					<form action="/dailyBoardComment/regComment" method="post" style="display: flex;justify-content: space-between;flex-wrap: nowrap;">
						<input type="hidden" name="boardNo" value="${vo.no}">
						<input type="hidden" name="nickname" value="${member.nickname}">
...
// 				console.log(content.nextElementSibling);
			...
			function modComment(no, boardNo, idx) {
				...
				$.ajax({
					url: '/dailyBoardComment/modComment',
					type: 'POST',
					dataType: 'text',
